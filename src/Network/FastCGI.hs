...
instance MonadFastCGI FastCGI where
    ...
    -- implementationBlockFastCGI action = do
      ...
      -- liftIO $ Exception.mask_ (runReaderT action state)
...
-- fBracket acquire release perform = do
  -- fBlock (do
           ...
           -- release resource
           -- return result)
-- fBracket acquire release perform = Exception.bracket acquire release perform
...
-- fFinally perform cleanup = do
  -- fBlock (do
           ...
           -- cleanup
           -- return result)
-- fFinally perform cleanup = Exception.finally perfrom cleanup
